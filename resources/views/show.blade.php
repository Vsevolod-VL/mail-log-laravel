...
@extends('mail-log::layout', ['title' => $group->subject ?: 'Gruppe'])
@section('content')
    <div class="border-b border-zinc-200 bg-white">
        <div class="mx-auto flex max-w-7xl items-center gap-3 px-6 py-3 text-sm">
            <a href="{{ route('mail-log.index') }}"
               class="inline-flex items-center gap-1.5 text-zinc-500 hover:text-zinc-900">
            ...
            <a href="{{ route('mail-log.index') }}"
               class="text-zinc-500 hover:text-zinc-900">{{ config('mail-log.ui.brand', 'Mail Log') }}</a>
            <span class="text-zinc-300">/</span>
            <span class="truncate font-medium">{{ $group->subject ?: '(kein Betreff)' }}</span>
            <span
                class="ml-2 inline-flex items-center rounded-md bg-zinc-100 px-1.5 py-0.5 font-mono text-[10px] uppercase text-zinc-500"
                title="Diese Seite stellt eine Gruppe dar; die Sendungen unten sind ihre Ereignisse.">Gruppe</span>

            <div class="ml-auto flex items-center gap-2">
                <form method="POST" action="{{ route('mail-log.destroy', $group) }}"
                      onsubmit="return confirm('Diese Gruppe und alle zugehörigen Ereignisse löschen?')">
                    @csrf
                    @method('DELETE')
                    <button
                        type="submit"
                        ...
                        title="Löscht die Gruppe und alle Sendungen darunter"
                    >
        ...
        <div class="space-y-6">
            ...
            <div>
                <div class="mb-2 flex items-center justify-between gap-2">
                    <h3 class="text-[11px] font-semibold uppercase text-zinc-500">Sendungen ({{ $events->total() }})</h3>
                </div>
                ...
                <div x-data="sendsTable()" class="rounded-lg border border-zinc-200 bg-white">
                    <table class="w-full text-sm">
                        ...
                        <tr class="border-b border-zinc-100 text-left text-[11px] font-medium uppercase text-zinc-500">
                            ...
                            <th class="px-3 py-2 font-medium">Empfänger</th>
                            <th class="px-3 py-2 font-medium">Gesendet</th>
                            <th class="px-3 py-2 font-medium">Dauer</th>
        ...
        <aside class="space-y-4">
            <div class="rounded-lg border border-zinc-200 bg-white">
                ...
                <dl class="divide-y divide-zinc-100 text-sm">
                    ...
                    <div class="grid grid-cols-3 gap-2 px-4 py-2">
                        <dt class="text-xs text-zinc-500">Fingerabdruck</dt>
